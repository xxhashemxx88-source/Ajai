  ...
  <div class="dashboard">
    ...
    <nav class="navbar">
      <div class="nav-brand">
        ...
        <span class="nav-name"
          >AJAI <span class="text-grad">Security</span></span
        >
      </div>
      ...
      <div class="nav-stats">
        <div class="nav-stat">
          ...
          >
            <path
              d="M15 10l4.553-2.553A1 1 0 0121 8.382v7.236a1 1 0 01-1.447.894L15 14M3 8a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8z"
            />
          </svg>
        ...
        <template v-if="alerts.newCount > 0">
          ...
          <div class="nav-stat">
            ...
            <span style="color: #f06080; font-weight: 700"
              >{{ alerts.newCount }} new</span
            >
          </div>
        </template>
      </div>
          ...
          >
            ...
            <div class="cell-footer">
              <div class="cell-info">
                <span
                  class="status-dot"
                  :class="cam.isActive ? 'status-active' : 'status-inactive'"
                ></span>
                <span class="cell-name"
                  >Unit-{{ String(cam.id).padStart(3, "0") }}</span
                >
                <span class="cell-email">{{ cam.email }}</span>
              </div>
              <div class="cell-actions" @click.stop>
                <span
                  v-if="cam.isActive && alerts.liveFrames[cam.email]"
                  class="live-pill"
                >
                ...
                <button
                  class="cell-toggle"
                  :class="cam.isActive ? 'ct-off' : 'ct-on'"
                  @click="cameras.toggleCamera(cam.id)"
                >
        ...
        <div class="panel-header">
          <div>
            ...
            <h3 class="panel-title">
              Threat Alerts
              <span v-if="alerts.newCount > 0" class="new-badge"
                >+{{ alerts.newCount }}</span
              >
            </h3>
          </div>
          <button
            v-if="alerts.newCount"
            class="clear-btn"
            @click="alerts.clearNewCount()"
          >
            <svg
              width="10"
              height="10"
        ...
        <div v-else class="alerts-scroll">
          ...
          >
            <div class="alert-thumb">
              <img
                v-if="alert.base64Image"
                :src="`data:image/jpeg;base64,${alert.base64Image}`"
                alt="threat"
              />
    ...
    >
      <div class="fullscreen-box">
        ...
        <div class="fs-header">
          <div class="fs-title">
            <span
              class="status-dot"
              :class="
                expandedCam.isActive ? 'status-active' : 'status-inactive'
              "
            ></span>
            ...
            <span
              v-if="
                expandedCam.isActive && alerts.liveFrames[expandedCam.email]
              "
              class="live-pill"
            >
              <span class="rec-dot"></span> LIVE
          ...
          <div class="fs-actions">
            <button
              class="cell-toggle"
              :class="expandedCam.isActive ? 'ct-off' : 'ct-on'"
              @click="cameras.toggleCamera(expandedCam.id)"
            >
        ...
        <div class="fs-feed">
          <img
            v-if="alerts.liveFrames[expandedCam.email]"
          ...
          <div v-else class="fs-empty">
            <svg
              ...
              fill="none"
              :stroke="
                expandedCam.isActive
                  ? 'rgba(57,201,110,0.2)'
                  : 'rgba(197,74,239,0.15)'
              "
              stroke-width="0.8"
            >
              <path
...
const activeCameras = computed(
  () => cameras.cameras.filter((c) => c.isActive).length,
);

function expandCam(cam) {
  expandedCam.value = cam;
}
...
onMounted(async () => {
  ...
  try {
    await startSignalR({
      ...
      onNewAlert: (alert) => alerts.addAlert(alert),
...
.nav-logo {
  ...
  background: linear-gradient(
    135deg,
    rgba(61, 159, 230, 0.15),
    rgba(202, 70, 253, 0.15)
  );
...
.nav-stats {
  ...
  gap: 12px;
  padding: 5px 14px;
  background: rgba(197, 74, 239, 0.05);
...
.new-dot {
  ...
  border-radius: 50%;
  background: #f06080;
  box-shadow: 0 0 6px rgba(240, 96, 128, 0.6);
...
.user-chip {
  ...
  gap: 7px;
  padding: 5px 12px;
  background: rgba(197, 74, 239, 0.08);
  border: 1px solid var(--border-dim);
  border-radius: 20px;
}
.user-avatar {
...
.refresh-btn {
  ...
  font-size: 11px;
  padding: 5px 12px;
  background: rgba(197, 74, 239, 0.06);
  border: 1px solid var(--border-dim);
  border-radius: 6px;
  color: var(--text-dim);
  cursor: pointer;
...
.refresh-btn:hover {
  border-color: rgba(197, 74, 239, 0.3);
  color: #c8c8e8;
}
...
/* Make first cam in 3-cam layout span full row */
.grid-3 .cam-cell:first-child {
  grid-column: span 2;
}
...
.cam-cell:hover {
  border-color: rgba(197, 74, 239, 0.4);
  box-shadow: 0 0 20px rgba(197, 74, 239, 0.1);
}
.cam-cell-active {
  border-color: rgba(57, 201, 110, 0.2);
}
...
.c {
  ...
  width: 14px;
  height: 14px;
  border-color: rgba(197, 74, 239, 0.5);
  border-style: solid;
  pointer-events: none;
}
.c-tl {
...
.ct-on {
  border-color: rgba(57, 201, 110, 0.35);
  color: #39c96e;
  background: rgba(57, 201, 110, 0.08);
}
.ct-on:hover {
  background: rgba(57, 201, 110, 0.18);
}
.ct-off {
...
.status-active {
  background: #39c96e;
  box-shadow: 0 0 6px rgba(57, 201, 110, 0.6);
...
.new-badge {
  ...
  border: 1px solid rgba(240, 96, 128, 0.3);
  color: #f06080;
  padding: 2px 8px;
  border-radius: 12px;
  animation: pa 1s infinite;
...
.spinner-purple {
  width: 15px;
  height: 15px;
  border: 2px solid rgba(197, 74, 239, 0.2);
  border-top-color: #c54aef;
  border-radius: 50%;
...
.alert-item {
  ...
  gap: 9px;
  padding: 8px 10px;
  background: rgba(240, 96, 128, 0.04);
  border: 1px solid rgba(240, 96, 128, 0.15);
  border-left: 3px solid #f06080;
  border-radius: 6px;
  cursor: pointer;
  transition: all 0.2s;
}
.alert-item:hover {
  background: rgba(240, 96, 128, 0.09);
}
...
.thumb-empty {
  width: 100%;
  height: 100%;
  background: rgba(240, 96, 128, 0.05);
  border: 1px dashed rgba(240, 96, 128, 0.2);
  display: flex;
  align-items: center;
  justify-content: center;
}
...
.sev-dot {
  ...
  border-radius: 50%;
  background: #f06080;
  box-shadow: 0 0 8px rgba(240, 96, 128, 0.5);
...
.fullscreen-box {
  ...
  flex-direction: column;
  max-height: 90vh;
  box-shadow: 0 0 60px rgba(197, 74, 239, 0.15);
}
